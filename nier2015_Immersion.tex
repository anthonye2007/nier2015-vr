...
There have been substantial amounts of VR research done in the past, but very little of it shown how VR can aid in software engineering. In this paper we present a vision of four software engineering activities that could benefit from VR.

We envision a code review tool that enables developers to interact with the software primarily via head movement and hand gestures. The tool distributes information around the user according to relevance.

We describe RiftSketch, a VR live coding environment that instantly updates user's surroundings as they code a 3D scene.

We describe a remote collaboration tool that lets remote developers be right next to each other.

We describe how VR could be used when creating applications with physical analogs to provide a simulation of those analogs.
...
[recap each application]

[paragraph on metaphorical programming]

...
Although simulation software is not uncommon, VR allows users to view, explore and manipulate simulated worlds in ways that would otherwise be awkward with typical 2D display hardware. 

For example, if a programmer was working on avionics software, the VR world could include a simulation of a plane with moving control surfaces, engines and a visualization of the aircraft's fuel system. 
The programmer could scale the virtual plane to a manageable size to give her an overview of its behaviour, scale it to normal size and walk around the plane to inspect a particular control surface or scale it to larger than life-size and transport herself into the guts of the plane to watch a particular fuel valve respond to the state of the program.
